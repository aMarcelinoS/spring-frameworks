...
public interface FuncionarioRepository extends PagingAndSortingRepository<Funcionario, Integer>, JpaSpecificationExecutor<Funcionario> {
	...
	List<Funcionario> findByNome(String nome); //<= Derivated Queries
	
	@Query("SELECT f FROM Funcionario f " //<= JPQL
			+ "WHERE f.nome = :nome "
			+ "AND f.salario >= :salario "
			+ "AND f.dataContratacao = :data")
	List<Funcionario> findNomeSalarioMaiorDataContratacao(String nome, Double salario, LocalDate data); 
	
	@Query(value = "SELECT * FROM funcionarios f WHERE f.data_de_contratacao >= :data", nativeQuery = true) //Query nativa do SQL
